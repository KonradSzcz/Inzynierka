...
import com.crypto.inzynierka.databinding.FragmentChapter4quizBinding

class Chapter4quiz : Fragment() {
    private var _binding: FragmentChapter4quizBinding? = null
    ...
    private fun loadQuestionsFromDB(chapter: Int) {
        ...
        val chapterColumn = "chapter4$chapter"  // Dynamiczne określenie kolumny rozdziału
    ...
    override fun onCreateView(
        ...
        _binding = FragmentChapter4quizBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (CurrentChapter < 4) {
            binding.nextChapter.setOnClickListener {
                replaceFragment(Chapter4(), CurrentChapter+1)
            }
        } else {
    ...
    private fun saveResult() {
        ...
        val cursor = db.rawQuery(
            ...
            arrayOf("chapter4", CurrentChapter.toString())
        )

        if (!cursor.moveToFirst()) {
            // Jeśli nie ma rekordu, wstaw nowy
            val values = ContentValues().apply {
                put(DBConnection.COL2_TESTS, "chapter4")
