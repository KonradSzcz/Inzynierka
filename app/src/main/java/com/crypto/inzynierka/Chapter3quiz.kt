...
import com.crypto.inzynierka.databinding.FragmentChapter3quizBinding

class Chapter3quiz : Fragment() {
    private var _binding: FragmentChapter3quizBinding? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Pobierz wartość rozdziału z argumentów przekazanych do fragmentu
        arguments?.let {
            CurrentChapter = it.getInt("CurrentChapter", 1)
        }

        // Wczytaj pytania w zależności od aktualnego rozdziału
        loadQuestionsFromDB(CurrentChapter)
    }

    private fun loadQuestionsFromDB(chapter: Int) {
        val db = dbHelper.readableDatabase
        val chapterColumn = "chapter3$chapter"  // Dynamiczne określenie kolumny rozdziału
        val cursor: Cursor = db.rawQuery("SELECT * FROM ${DBConnection.TABLE_NAME_QUESTIONS} WHERE ${DBConnection.COL3_QUESTIONS} = '$chapterColumn'", null)
    ...
    override fun onCreateView(
        ...
        _binding = FragmentChapter3quizBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (CurrentChapter < 9) {
            binding.nextChapter.setOnClickListener {
                replaceFragment(Chapter3(), CurrentChapter+1)
            }
        } else {
    ...
    private fun saveResult() {
        ...
        val cursor = db.rawQuery(
            ...
            arrayOf("chapter3", CurrentChapter.toString())
        )

        if (!cursor.moveToFirst()) {
            // Jeśli nie ma rekordu, wstaw nowy
            val values = ContentValues().apply {
                put(DBConnection.COL2_TESTS, "chapter3")
